...
\textit{Training-based defenses} are our primary point of comparison. R2D2~\cite{mazeika2024harmbench}, introduced alongside HarmBench, is the canonical adversarial fine-tuning baseline: it trains on adversarial prompt--refusal pairs generated online during training, optimizing standard cross-entropy. This is our primary empirical baseline; it improves ASR but does not explicitly regularize the model against the \emph{directional shift} in unsafe probability that jailbreaks exploit. Short Circuiting (``Circuit Breakers``)~\cite{zou2024circuitbreakers} take a different route: a low-rank representation adaptation is trained to reroute harmful hidden states against a retain set, achieving 87--90\% ASR reduction on HarmBench with under 1\% utility loss on MT-Bench. This is the closest training-time peer to our work; the key distinction is that Circuit Breakers intervene in activation space, whereas we intervene in token-probability space through an output-side consistency objective. SmoothLLM~\cite{robey2023smoothllm} applies randomized smoothing at inference by character-level perturbing the prompt and aggregating outputs; it shares our intuition that safety should be invariant to perturbations but acts at inference time and pays a non-trivial utility tax, whereas directed consistency learning bakes the invariance into the LoRA weights without inference overhead. Latent Adversarial Training (LAT)~\cite{sheshadri2024lat} perturbs residual streams  during fine-tuning, improving robustness to persistent harmful behaviours; it intervenes  in activation space rather than supervising through an output-side safety critic.  Continuous adversarial training approaches~\cite{xhonneux2024afficientAT} apply PGD-style embedding-space perturbations during fine-tuning; however, \citet{schwinn2024revisiting} show that representation-engineering defenses including Circuit Breakers remain vulnerable to small embedding-space tweaks, motivating output-probability-space defenses such as ours. MixAT~\cite{dekany2025mixat} combines continuous embedding perturbations with discrete adversarial inputs and is the closest training-time comparator to our work: it reports under 20\% attack success on an  At-Least-One metric across attack families. The key distinction is that MixAT supervises via cross-entropy on harmful-prompt targets without a safety critic, whereas our directed stability term penalizes increases in guard-estimated unsafe probability relative to the clean anchor, an asymmetric objective that is silent whenever a perturbation does not worsen safety. DOOR~\cite{zhao2025door} decomposes the alignment  objective into a robust refusal term and targeted harmful-knowledge unlearning, with  reward-based token-level weighting; it is the closest loss structure to our directed stability term, but operates over preference pairs without a rollout or safety critic.
%
...
Each training row has a harmful prompt $h_n$, an adversarial rewrite $h'_n$ (from GCG, AutoDAN, or PAIR; see Section~\ref{sec:exp-adversarial}), and a continuation string $r_n$ taken from the \texttt{Original Response} column. Those targets were produced offline by querying a chat model on each $h_n$ and logging the reply; for Llama-2-7B-Chat the replies are almost always refusals, but the pipeline does not assert that---it simply minimises cross-entropy on the stored text. We write $\mathcal{D}=\{(h_n,h'_n,r_n)\}_{n=1}^{N}$ and denote the trainable chat model by $\pi_\theta$.
...
\subsection{Main results: seen-family evaluation}
\label{sec:results-seen}

Table~\ref{tab:main-results} reports per-benchmark ASR and FRR (\%) for Llama-2-7B-Chat under the seen-family protocol; each table cell lists ASR$\,|\,$FRR for AdvBench, HarmBench, and JailbreakBench respectively. Metrics use the best $(\lambda,\varepsilon)$ selected on the seen-family validation surface (Section~\ref{sec:exp-setup}). We compare the base checkpoint, adversarial SFT (\texttt{perturbed}, $\lambda{=}0$), and directed consistency learning (DCL, our method).


%%%% PLEASE REPLACE/REMOVE THIS, THIS IS JUST A TEMPLATE 
\begin{table}[h]
\centering
\caption{Seen-family evaluation on Llama-2-7B-Chat. Each benchmark column reports ASR$\,|\,$FRR (\%); lower is better for both. Row blocks slice by attack family used at test time (\textit{All attacks} pools families). Best DCL $(\lambda,\varepsilon)$ is used throughout; best values \textbf{bolded}.}
\label{tab:main-results}
\small
\setlength{\tabcolsep}{5pt}
\begin{tabular}{lccc}
\toprule
\textbf{Method} & \textbf{AdvBench} & \textbf{HarmBench} & \textbf{JailbreakBench} \\
 & {\footnotesize ASR$\downarrow\,|\,$FRR$\downarrow$} & {\footnotesize ASR$\downarrow\,|\,$FRR$\downarrow$} & {\footnotesize ASR$\downarrow\,|\,$FRR$\downarrow$} \\
\midrule
\multicolumn{4}{c}{\textit{All attacks (aggregate)}} \\
\midrule
Base       & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} \\
Adv.\ SFT  & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} \\
DCL (ours) & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} \\
\midrule
\multicolumn{4}{c}{\textit{GCG}} \\
\midrule
Base       & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} \\
Adv.\ SFT  & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} \\
DCL (ours) & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} \\
\midrule
\multicolumn{4}{c}{\textit{AutoDAN}} \\
\midrule
Base       & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} \\
Adv.\ SFT  & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} \\
DCL (ours) & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} \\
\midrule
\multicolumn{4}{c}{\textit{PAIR}} \\
\midrule
Base       & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} \\
Adv.\ SFT  & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} \\
DCL (ours) & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} \\
...
Table~\ref{tab:unseen-results} reports per-benchmark ASR and FRR (\%) when the held-out attack family was entirely absent during training; each cell lists ASR$\,|\,$FRR for AdvBench, HarmBench, and JailbreakBench. The same $(\lambda,\varepsilon)$ selected from the seen-family surface is reused without re-tuning.

\begin{table}[h]
\centering
\caption{Unseen-family generalisation on Llama-2-7B-Chat. The adapter is trained without the listed held-out attack family; tests use attacks from that held-out family only. Columns are benchmarks; each entry is ASR$\,|\,$FRR (\%). Lower is better. Best values \textbf{bolded}.}
...
\textbf{Method} & \textbf{AdvBench} & \textbf{HarmBench} & \textbf{JailbreakBench} \\
 & {\footnotesize ASR$\downarrow\,|\,$FRR$\downarrow$} & {\footnotesize ASR$\downarrow\,|\,$FRR$\downarrow$} & {\footnotesize ASR$\downarrow\,|\,$FRR$\downarrow$} \\
\midrule
\multicolumn{4}{c}{\textit{Hold-out: GCG}} \\
\midrule
Base       & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} \\
Adv.\ SFT  & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} \\
DCL (ours) & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} \\
\midrule
\multicolumn{4}{c}{\textit{Hold-out: AutoDAN}} \\
\midrule
Base       & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} \\
Adv.\ SFT  & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} \\
DCL (ours) & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} \\
\midrule
\multicolumn{4}{c}{\textit{Hold-out: PAIR}} \\
\midrule
Base       & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} \\
Adv.\ SFT  & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} \\
DCL (ours) & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} & \texttt{TODO}\,$|$\,\texttt{TODO} \\
...
\label{sec:results-perbenchmark}

Figure~\ref{fig:per-benchmark} shows the per-benchmark ASR for Llama-2-7B-Chat under the seen-family protocol to surface whether gains are consistent across AdvBench, HarmBench, and JailbreakBench or concentrated in one dataset.

\begin{figure}[h]
\centering
% \includegraphics[width=\linewidth]{figures/per_benchmark_asr.pdf}
\fbox{\rule{0pt}{4cm}\rule{0.9\linewidth}{0pt}} % placeholder: replace with actual figure
\caption{Per-benchmark ASR (\%) under seen-family evaluation for Llama-2-7B-Chat. Three grouped bars per dataset (Base / Adv.\ SFT / DCL). \texttt{[PLACEHOLDER --- replace with figures/per\_benchmark\_asr.pdf]}}
\label{fig:per-benchmark}
\end{figure}

\subsection{Ablation, hyperparameter sensitivity}
\label{sec:results-hparam}

Figure~\ref{fig:hparam-heatmap} shows aggregate seen-family ASR as a function of $\lambda$ and $\varepsilon$ for Llama-2-7B-Chat. Darker cells indicate lower (better) ASR.

\begin{figure}[h]
\centering
% \includegraphics[width=0.6\linewidth]{figures/hparam_heatmap_llama2.pdf}
\fbox{\rule{0pt}{4cm}\rule{0.6\linewidth}{0pt}} % placeholder
\caption{Seen-family ASR heatmap over the $(\lambda, \varepsilon)$ grid for Llama-2-7B-Chat. Darker $=$ lower ASR (better). Dashed box marks the selected operating point. \texttt{[PLACEHOLDER --- replace with figures/hparam\_heatmap\_llama2.pdf]}}
\label{fig:hparam-heatmap}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Discussion and Furutre Directions}
\label{sec:discussion}
TODO: Talk about the results here

\paragraph{Multi-turn attacks require rethinking the anchor.}
Crescendo~\cite{russinovich2024crescendo} and related strategies work precisely because no individual turn looks harmful; the problem is the trajectory. Applying directed consistency learning here is non-trivial because the ``anchor'' is no longer a single prompt but a conversation prefix, and an adversarial neighbour would need to be a prefix-level perturbation rather than a prompt rewrite. Defining what a semantically equivalent but adversarially shifted conversation looks like, and building a data pipeline to generate such neighbours at scale, is probably the most practically important extension of this work, given how prevalent multi-turn attacks are becoming against deployed systems.
...
Jailbreak vulnerability is a consistency failure: a model that refuses a direct harmful request but complies with a lightly perturbed variant already possesses the relevant refusal behaviour, what it lacks is stability under perturbation. This framing motivates a training objective that directly targets the gap, penalizing directional increases in unsafe response probability across adversarially perturbed prompt pairs rather than simply augmenting the training distribution and optimizing cross-entropy on refusals. Directed consistency learning instantiates this objective through three design choices that make it tractable: an asymmetric hinge that is silent whenever a perturbation does not worsen safety, a frozen safety critic that provides a differentiable signal without online rollouts or reward-model training, and a soft autoregressive rollout that preserves gradient flow through discrete generation. Applied via LoRA on Llama-2-7B-Chat, the method reduces ASR relative to both the base model and adversarial supervised fine-tuning while maintaining a competitive false refusal rate. The most notable result is the held-out-family generalization: ASR reduction persists on attack families withheld entirely from the consistency objective during training. If the method were simply memorizing seen attack surfaces, this gap should be both larger and more attack-specific than we observe. We take this as evidence that the asymmetric loss captures something structural about the direction of unsafe probability drift — a property that generalizes across attack families because it targets the underlying consistency failure rather than its surface manifestations. Extending this to multi-turn attacks, where the anchor is a conversation prefix rather than a single prompt, is an important open direction.
