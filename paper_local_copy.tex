...
\usepackage{array}
\usepackage{enumitem}
% natbib loaded by neurips_2026; options set via \PassOptionsToPackage above.
% Use \missref{key} only for citations not yet in references.bib (shows red placeholder).
...
\begin{abstract}
Aligned LLMs usually refuse plain harmful requests, but small changes to the prompt can still elicit unsafe answers. We fine-tune models with \emph{directed consistency learning}: we ask the model to stay at least as safe on perturbed harmful prompts as on the originals, and we score ``how unsafe'' the partial answer looks with a frozen safety critic. Because sampling tokens breaks gradients, we train with a short \emph{soft} rollout that averages token embeddings instead of picking a single token at each step. We use 3{,}000 harmful training prompts and adversarial neighbours from GCG, AutoDAN, and PAIR, and we test on AdvBench, HarmBench, and JailbreakBench under \emph{seen-family} attacks (all three families at test time) and \emph{unseen-family} attacks (one family held out of the hinge during training, then tested in isolation). On Llama-2, Llama-3, and Mistral backbones, this recipe lowers attack success rate (ASR) compared to the base model and a standard adversarial fine-tuning baseline, without large spikes in false refusal rate (FRR). Overall, nudging consistency across adversarial rewrites of the same harmful intent is a simple lever for jailbreak robustness.
\end{abstract}
