#!/usr/bin/env bash
# Shared body for Vulcan (aip-mijungp) base-model test eval (Llama-2 / Llama-3 / Mistral).
#
# Prerequisites:
#   • export MODEL_PROFILE to one of: llama_2_7b_chat, llama_3_8b_instruct, mistral_7b_instruct
#   • HF weights + judges under $SCRATCH/hf_models/ (see REFUSAL_JUDGE_PATH /
#     JAILBREAK_CLASSIFIER_PATH below -- train/model_profiles.py's built-in
#     defaults are Narval-shaped and will be wrong on Vulcan)
#
# Optional env:
#   REPO_ROOT              default: ${HOME}/repos/dp-llm-experiments (code lives on
#                          $HOME on Vulcan, not $SCRATCH -- see delman_eval.sh's precedent)
...
#   BASE_LLM               override profile.base_llm with an arbitrary HF repo/path
#                          (e.g. a third-party baseline checkpoint like MixAT/DOOR).
#                          Requires the checkpoint to share the profile's architecture/tokenizer.
#                          For a merged/full checkpoint (e.g. DOOR), this is all you need.
#   BASE_LORA_PATH         optional LoRA/PEFT adapter dir loaded on top of BASE_LLM via
#                          eval.py's --resume-from (e.g. MixAT, which HF only distributes
#                          as an adapter over the base Instruct model, never merged).
#                          Leave unset for merged/full checkpoints.
#   BASELINE_TAG           output subdir/run-tag suffix when BASE_LLM is set
#                          (default: sanitized BASE_LLM); keeps baselines from colliding
#                          with the profile's own base-model outputs.
#
...
# model_profiles.py's eval_refusal_judge_path / eval_jailbreak_classifier_path
# defaults are Narval-shaped ($SCRATCH/mistral_7b_instruct etc.); Vulcan keeps
# HF snapshots under $SCRATCH/hf_models/ instead (see delman_eval.sh's precedent).
...
if [[ -n "${BASE_LORA_PATH:-}" && ! -d "${BASE_LORA_PATH}" ]]; then
  echo "ERROR: BASE_LORA_PATH not found: ${BASE_LORA_PATH}" >&2
  exit 2
fi
...
if [[ -n "${BASE_LLM:-}" ]]; then
  DEFAULT_BASELINE_TAG="$(echo "${BASE_LLM}" | tr '/: ' '___')"
  BASELINE_TAG="${BASELINE_TAG:-${DEFAULT_BASELINE_TAG}}"
  OUT_DIR="${BASE_MODEL_TEST_OUT_DIR}/${MODEL_PROFILE}_${BASELINE_TAG}"
  RUN_TAG="base_test_${MODEL_PROFILE}_${BASELINE_TAG}"
else
...
EVAL_MODE="${EVAL_MODE:-seen-family}"
...
if [[ -n "${BASE_LLM:-}" ]]; then
  echo "BASE_LLM override=${BASE_LLM} (baseline_tag=${BASELINE_TAG})"
fi
if [[ -n "${BASE_LORA_PATH:-}" ]]; then
  echo "BASE_LORA_PATH=${BASE_LORA_PATH} (adapter loaded on top of BASE_LLM via --resume-from)"
fi
...
if [[ -n "${HARMFUL_OUT}" && -n "${BENIGN_OUT}" ]]; then
  ...
  EVAL_EXTRA_ARGS=()
  if [[ -n "${BASE_LLM:-}" ]]; then
    EVAL_EXTRA_ARGS+=(--base-llm "${BASE_LLM}")
  fi
  if [[ -n "${BASE_LORA_PATH:-}" ]]; then
    EVAL_EXTRA_ARGS+=(--resume-from "${BASE_LORA_PATH}")
  fi
