#!/bin/bash
#SBATCH --job-name=baseline_eval_nosys
#SBATCH --account=rrg-mijungp
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=6
#SBATCH --mem=40G
#SBATCH --time=7:00:00
#SBATCH --output=output/baseline_eval_nosys_%j.out
...
echo "Evaluating baseline model (no LoRA adapter), Protocol-Undefended (no system prompt)"
...
if [[ "$SKIP_EVAL" -eq 0 ]]; then
  ...
  BENIGN_TMP="${SLURM_TMPDIR}/frr_eval_input_baseline_nosys.csv"
...
if "Original Prompt" in df.columns:
    ...
    print(f"[baseline_eval_nosys] benign data already has 'Original Prompt' -> {dst}")
    sys.exit(0)
...
df.to_csv(dst, index=False)
print(f"[baseline_eval_nosys] mapped '{src_col}' -> 'Original Prompt' -> {dst}")
PY
  BENIGN_DATA_FOR_EVAL="$BENIGN_TMP"

  # No --resume-from -> baseline (bare Llama-2-7B-Chat).
...
echo "Baseline (no-sys-prompt) eval complete."
echo "Outputs:"
echo "  $HARMFUL_OUT"
echo "  $BENIGN_OUT"
echo "  $METRICS_OUT"
